...
import { useAppContext, type PotteryEntry } from '../App';
...
export function CommerceMarketplace() {
  ...
  const [marketplaceItems] = useState<(PotteryEntry & { artistName: string; artistAvatar?: string })[]>([
    {
      id: '1',
      date: '2025-06-03',
      ...
      isForSale: true,
      price: 85,
      description: 'Unique hand-built vase with beautiful copper green glaze. Perfect for dried flowers or as a decorative piece.',
      dimensions: '8" H x 4" W',
      artistName: 'Jane Potter',
      artistAvatar: 'https://images.unsplash.com/photo-1494790108755-2616b332c35-?w=100&h=100&fit=crop&crop=face'
    },
    {
      id: '2',
      date: '2025-06-01',
      ...
      isForSale: true,
      price: 120,
      description: 'Beautiful set of 4 handmade ceramic bowls. Perfect for everyday dining or special occasions.',
    ...
    },
    {
      ...
      isForSale: true,
      price: 65,
      description: 'Striking decorative platter featuring geometric patterns in iron red and matte white glazes.',
  ...
  const getCommissionInfo = (price: number) => {
    ...
    const commission = (price * currentStudio.commissionRate) / 100;
        ...
        <div className="flex items-center space-x-4">
          ...
          {currentStudio && (
            <Badge className="bg-blue-100 text-blue-800">
              Studio Commission: {currentStudio.commissionRate}%
            </Badge>
          )}
        </div>
      </div>
            ...
            <Card key={item.id} className="overflow-hidden hover:shadow-lg transition-shadow">
              ...
              <CardContent className="p-4">
                ...
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <span className="text-sm text-muted-foreground">by {item.artistName}</span>
                </div>
                ...
                {currentUser?.type === 'studio_admin' && currentStudio && (
                  <div className="text-xs bg-muted p-2 rounded mb-3">
                    <div className="flex justify-between">
                      <span>Artist receives:</span>
                      <span>${getCommissionInfo(item.price || 0)?.artistReceives.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Studio commission:</span>
                      <span>${getCommissionInfo(item.price || 0)?.commission.toFixed(2)}</span>
                    </div>
                  </div>
                )}
          ...
          <CardContent>
            ...
            <div className="flex flex-wrap gap-4 text-sm">
              <div className="flex items-center space-x-2">
                ...
                <span>{currentStudio.members.length} Artists</span>
              </div>
              <div className="flex items-center space-x-2">
                <Palette className="w-4 h-4" />
                <span>{currentStudio.glazes.length} Available Glazes</span>
              </div>
              <div className="flex items-center space-x-2">
                <DollarSign className="w-4 h-4" />
                <span>{currentStudio.commissionRate}% Studio Commission</span>
              </div>
            </div>
          </CardContent>
        </Card>
