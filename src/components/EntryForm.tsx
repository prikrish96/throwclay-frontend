...
import type { PotteryEntry, PhotoEntry } from '../App';
...
const potteryTypes = [
  'Bowl', 'Mug', 'Vase', 'Plate', 'Cup', 'Pitcher', 'Sculpture', 'Tile', 'Pot', 'Other'
];

const clayTypes = [
  'Earthenware', 'Stoneware', 'Porcelain', 'Raku', 'Paper Clay', 'Terracotta', 'Other'
];
...
const firingTypes = [
  'Bisque', 'Glaze', 'Raku', 'Pit firing', 'Saggar', 'Wood firing', 'Gas reduction', 'Electric'
];

const glazeOptions = [
  'Celadon', 'Iron Red', 'Copper Green', 'Cobalt Blue', 'Matte White', 'Glossy Clear',
  'Tenmoku', 'Shino', 'Ash Glaze', 'Crystalline', 'Metallic', 'Underglaze'
];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select value={formData.potteryType} onValueChange={(value) => setFormData(prev => ({ ...prev, potteryType: value }))}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Select value={formData.clayType} onValueChange={(value) => setFormData(prev => ({ ...prev, clayType: value }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select clay type" />
                  </SelectTrigger>
                  <SelectContent>
                    {clayTypes.map(clay => (
                      <SelectItem key={clay} value={clay}>{clay}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-wrap gap-2">
              {formData.techniques.map(technique => (
                <Badge key={technique} variant="secondary" className="flex items-center space-x-2">
                  <span>{technique}</span>
                  <button
                    type="button"
            ...
            <div className="flex space-x-2">
              <Input
                ...
                onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addTechnique(newTechnique))}
              />
              <Button type="button" onClick={() => addTechnique(newTechnique)}>
                Add
              </Button>
            </div>
          </CardContent>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select value={formData.firingType} onValueChange={(value) => setFormData(prev => ({ ...prev, firingType: value }))}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, firingTemp: e.target.value }))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {formData.glazes.map(glaze => (
                  <Badge key={glaze} variant="secondary" className="flex items-center space-x-2">
                    <span>{glaze}</span>
                    <button
                      type="button"
              ...
              <div className="flex space-x-2 mt-2">
                <Input
                  ...
                  onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addGlaze(newGlaze))}
                />
                <Button type="button" onClick={() => addGlaze(newGlaze)}>
                  Add
                </Button>
              </div>
            </div>
        ...
        {/* Photos & Sketches */}
        <PhotoManager photos={formData.photos} onPhotosChange={handlePhotosChange} />
